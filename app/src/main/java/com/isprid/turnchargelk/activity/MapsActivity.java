...
public class MapsActivity extends AppCompatActivity implements OnMapReadyCallback, GoogleMap.OnMarkerClickListener, GoogleMap.OnMapClickListener {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(ACTION_OBD_CONNECTION_STATUS)) {
                ...
                if (connectionStatusMsg.equals(getString(R.string.obd_connected))) {
                    ...
                    stopService(new Intent(MapsActivity.this, ObdReaderService.class));
                } else {
                ...
                TripRecord tripRecord = TripRecord.getTripRecode(MapsActivity.this);

                try {
                    if (ObdHelper.getBatteryLevel() != -1) {
                        if (Integer.parseInt(tripRecord.getEngineRpm()) > 3500) {
                            intent.putExtra(Constant.EXTRA_DRAIN, 2);
    ...
    LocationCallback mLocationCallback = new LocationCallback() {
        ...
        public void onLocationResult(LocationResult locationResult) {
            ...
            if (locationList.size() > 0) {
                ...
                double distance = SphericalUtil.computeDistanceBetween(new LatLng(location.getLatitude(), location.getLongitude()), destination.getLocation());

                Log.e("TAG", "Distance between A & B is \n " + String.format("%.2f", distance / 1000) + "km");

                checkDrivePossibility(distance);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Objects.requireNonNull(getSupportActionBar()).setTitle("Charging Stations");
        ...
        destination = (MyDestination) getIntent().getExtras().getSerializable(Constant.EXTRA_DESTINATION);

        Log.e("Destination", "Name: " + destination.getName() + " | Latitude: " + destination.getLatitude() + " | Longitude: " + destination.getLongitude());
        ...
        text_destination.setText(destination.getName());
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.action_connect) {
            startService(new Intent(MapsActivity.this, ObdReaderService.class));
            return true;
        }
    ...
    private void drawDestination() {
        try {
            mSelectedLatLang = new LatLng(destination.getLatitude(), destination.getLongitude());
            btnDirection.setEnabled(false);
            drawLocation();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void setDestination() {
        ...
        MarkerOptions markerOptions = new MarkerOptions();
        markerOptions.position(latLng);
        markerOptions.title(destination.getName());
        markerOptions.icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_RED));
        mGoogleMap.addMarker(markerOptions);
    }

    private void setEVLocations() {
        list = new ArrayList<>();
//        list.addAll(EVDataProvider.getChargingStationList());
//        pinLocationsToMap(list);
    ...
    private void checkDrivePossibility(double distance) {
        try {
            Log.e("Possibility", "Distance between A & B " + (distance / 1000));
            Log.e("Possibility", "Driving Distance " + ObdHelper.getDrivingDistance());
            if (ObdHelper.getDrivingDistance() < (distance / 1000)) {
                ...
                AlertDialog alertDialog = new AlertDialog.Builder(this)
                        .setTitle("Warning!")
                        .setMessage("The remaining battery is not enough to reach the destination. Therefore charge the battery from the " + evPlace.getName() + " charging station.")
    ...
    public boolean onMarkerClick(Marker marker) {
        ...
        if (marker.getTitle().equals("Current Location")) {
            ...
            if (evPlace != null) {
                ...
                text_name.setText(destination.getName());
